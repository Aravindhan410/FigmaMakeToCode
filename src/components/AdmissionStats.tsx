...
export default function AdmissionStats() {
  ...
  const totals = courseData.reduce(
    (acc, course) => ({
      ...
      ohc: acc.ohc + course.ohc,
      sc: acc.sc + course.sc,
      st: acc.st + course.st,
      noOfVacant: acc.noOfVacant + course.noOfVacant,
    }),
    { admitted: 0, oc: 0, bc: 0, mbc: 0, ohc: 0, sc: 0, st: 0, noOfVacant: 0 }
  );
  ...
  return (
    <>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="sm:max-w-[480px] bg-gradient-to-b from-[#c5d5e8] to-[#e8eef5] border-2 border-[#6b8fb8] shadow-xl">
          <DialogHeader className="relative pb-4 border-b border-gray-300">
            <DialogTitle className="text-lg text-gray-800">Statistics</DialogTitle>
            <DialogDescription className="sr-only">
              Enter the year for admission statistics report
            </DialogDescription>
            <button
              onClick={handleCancel}
              className="absolute right-0 top-0 w-6 h-6 flex items-center justify-center hover:bg-gray-300 rounded"
            >
              <X className="w-4 h-4" />
            </button>
          </DialogHeader>
          ...
          <div className="py-6">
            <Label htmlFor="year" className="text-base text-gray-800 mb-4 block">
              Enter year.?
            </Label>
            <Input
              id="year"
              ...
              placeholder=""
              className="w-full h-10 border-t-0 border-l-0 border-r-0 border-b-2 border-[#4a7ba7] bg-transparent rounded-none focus:outline-none focus:border-[#2d5a8a] focus-visible:ring-0 focus-visible:ring-offset-0"
              onKeyDown={(e) => {
          ...
          <DialogFooter className="flex gap-2 sm:justify-end pt-4">
            <Button
              onClick={handleOk}
              className="min-w-[80px] h-9 bg-[#e8eef5] hover:bg-[#d0dae8] text-gray-800 border border-gray-400 shadow-sm"
            >
              OK
            </Button>
            <Button
                        ...
                        <table className="w-full border-collapse">
                          <thead>
                            ...
                            <tr>
                              ...
                              <th className="border-2 border-black p-2 text-sm">OHC</th>
