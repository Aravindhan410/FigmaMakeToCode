...
export default function ExportAbsentees() {
  const [activeTab, setActiveTab] = useState('absenteelist');
  ...
  const handleClose = () => {
    console.log('Closing Export Absentees');
  };
      ...
      <div className="bg-[#525252] rounded p-4 min-h-[500px] overflow-auto">
        ...
        >
          ...
          <div className="text-center mb-6">
            <h3 className="text-xl">EXPORT ABSENTEES REPORT</h3>
            <p className="text-sm text-gray-600 mt-2">Examination Absentees List</p>
          </div>
          ...
          <table className="w-full border-collapse border border-gray-400 text-sm">
            ...
            <tbody>
              {Array.from({ length: 12 }, (_, idx) => ({
                sno: idx + 1,
                regno: `RP2025${String(2501 + idx).padStart(4, '0')}`,
                name: `Student ${String.fromCharCode(65 + (idx % 26))} ${Math.floor(idx / 26) + 1}`,
                course: idx % 3 === 0 ? 'B.Pharm' : idx % 3 === 1 ? 'D.Pharm' : 'M.Pharm',
                year: ['I', 'II', 'III', 'IV'][idx % 4],
                subject: ['Pharmaceutics', 'Pharmacology', 'Pharmaceutical Chemistry'][idx % 3],
                reason: idx % 2 === 0 ? 'Medical' : 'Personal',
              })).map((row, idx) => (
                <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="border border-gray-400 p-2">{row.reason}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="mt-8 pt-6 border-t border-gray-300">
            <div className="grid grid-cols-2 gap-8 text-sm">
              <div>
                <p>Total Absentees: <strong>12</strong></p>
    ...
    <div className="min-h-screen bg-[#8a9aad] p-6">
      <div className="w-full max-w-[1150px] mx-auto">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="absenteelist" className="mt-0">
            {renderTabContent()}
          </TabsContent>

          <TabsContent value="subjectwise" className="mt-0">
            {renderTabContent()}
          </TabsContent>

          <TabsContent value="coursewise" className="mt-0">
            {renderTabContent()}
          </TabsContent>
        </Tabs>
