import svgPaths from "./svg-m503noi1m4";
...
function StaffDetailsForm18({ activeTab, onTabChange }: { activeTab: 'staff' | 'subject'; onTabChange: (tab: 'staff' | 'subject') => void }) {
  return (
...
function App({ activeTab, onTabChange }: { activeTab: 'staff' | 'subject'; onTabChange: (tab: 'staff' | 'subject') => void }) {
  return (
    <div className="bg-gray-100 box-border content-stretch flex flex-col h-[1368px] items-start pb-0 pt-[16px] px-[16px] relative shrink-0 w-[1150px]" data-name="App">
      <StaffDetailsForm18 activeTab={activeTab} onTabChange={onTabChange} />
    </div>
  );
}

export default function StaffDetails({ activeTab, onTabChange }: { activeTab?: 'staff' | 'subject'; onTabChange?: (tab: 'staff' | 'subject') => void }) {
  return (
    <div className="bg-white content-stretch flex flex-col items-start relative size-full" data-name="staff details">
      <App activeTab={activeTab || 'staff'} onTabChange={onTabChange || (() => {})} />
    </div>
  );
}